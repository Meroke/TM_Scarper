  ...
  const readSelectedSku = () => {
    ...
    for (const selector of selectors) {
      for (const node of document.querySelectorAll(selector)) {
        const text = (node.innerText || node.textContent || node.title || "").trim();
    ...
    const scopedText = (scopedSelected?.innerText || scopedSelected?.textContent || "").trim();
    if (scopedText && !values.includes(scopedText)) values.unshift(scopedText);
